...
Description messages based on the following [https://www.gurobi.com/documentation/9.5/refman/error_codes.html#sec:ErrorCodes:](https://www.gurobi.com/documentation/9.5/refman/error_codes.html#sec:ErrorCodes).
...
| INDEX_OUT_OF_RANGE       | 10006        | Tried to query or set an attribute but one or more of the provided indices is invalid                            | gurobipy_exceptions.GRBIndexOutOfRange           |
...
| JOB_REJECTED             | 10023        | Gurobi Compute Server responded but was unable to process the job                                                | gurobipy_exceptions.GRBJobRejected               |
| NOT_SUPPORTED            | 10024        | A Gurobi feature is not supported under your usage environment                                                   | gurobipy_exceptions.GRBNotSupported              |
| EXCEED_2B_NONZEROS       | 10025        | A query routine has been called on a model with more than 2 billion non-zero entries                             | gurobipy_exceptions.GRBExceed2bNonzeros          |
| INVALID_PIECEWISE_OBJ    | 10026        | Violated property for Piecewise-linear objective                                                                 | gurobipy_exceptions.GRBInvalidPiecewiseObj       |
...
| MODEL_MODIFICATION       | 10029        | The model has been modified and now is invalid.                                                                  | gurobipy_exceptions.GRBModelModification         |
| CSWORKER                 | 10030        | That there was an application problem client-server feature                                                      | gurobipy_exceptions.GRBCsworker                  |
| TUNE_MODEL_TYPES         | 10031        | Tuning was invoked on a set of models but the models were of different types (e.g. one an LP another a MIP).     | gurobipy_exceptions.GRBTuneModelTypes            |
| SECURITY                 | 10032        | Authentication step failed or current credentials for attempted operation.                                       | gurobipy_exceptions.GRBSecurity                  |
| NOT_IN_MODEL             | 20001        | Constraint or variable not in the model (either removed or not yet been added.                                   | gurobipy_exceptions.GRBNotInModel                |
